...
tsukamoto::tsukamoto(int argc, char** argv)
{
    using namespace fl;
    ...
    // Motoru yapılandır
    engine->configure("Minimum", "Maximum", "Minimum", "Maximum", "Centroid","First");

    X->setValue(5.0);

    engine->process();
    double rampsValue = Ramps->getValue();
    double sigmoidsValue = Sigmoids->getValue();
    double zsShapesValue = ZSShapes->getValue();
    double concavesValue = Concaves->getValue();

    std::cout << "Ramps: " << rampsValue << std::endl;
    std::cout << "Sigmoids: " << sigmoidsValue << std::endl;
    std::cout << "ZSShapes: " << zsShapesValue << std::endl;
    std::cout << "Concaves: " << concavesValue << std::endl;
